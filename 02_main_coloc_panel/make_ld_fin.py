...
eur = set(open(f"{DIR}/fin_unrel.txt").read().split())
vcf = pysam.VariantFile(f"{DIR}/kgp_chr15_window.vcf")
samples = list(vcf.header.samples)
keep = [s for s in samples if s in eur]
print(f"VCF samples={len(samples)}, FIN unrel kept={len(keep)}")
...
var = Gok.var(axis=1)
mono = var == 0
print(f"monomorphic in FIN: {mono.sum()}")
Gok = Gok[~mono]
idx = np.where(ok)[0][~mono]
D = np.corrcoef(Gok)
np.save(f"{DIR}/ld_fin.npy", D)
univ.iloc[idx][["chromosome", "position", "ref", "alt", "rsid"]].to_csv(
    f"{DIR}/ld_fin_snps.csv", index=False)
