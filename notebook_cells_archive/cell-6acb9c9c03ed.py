...
df = pd.read_csv('/workspace/coloc_panel/coloc_panel_results.csv')
ok = df[df.note == 'ok'].copy()

ds_order = ['GTExv8_WholeBlood', 'GTExv8_SkinSE', 'GTExv8_SkinNSE', 'BLUEPRINT_Neutro', 'BLUEPRINT_Mono']
ds_label = {'GTExv8_WholeBlood': 'GTEx v8 whole blood (n=670)',
...
colors = {'GTExv8_WholeBlood': '#0279EE', 'GTExv8_SkinSE': '#FF9400', 'GTExv8_SkinNSE': '#F5C98A',
          'BLUEPRINT_Neutro': '#75A025', 'BLUEPRINT_Mono': '#B5D48A'}

fig, axes = plt.subplots(1, 2, figsize=(11.5, 5.6), sharex=True)
    ...
    for ds in ds_order:
        for g in gene_order:
            ...
            if len(row) == 0:
                continue
            r = row.iloc[0]
            ax.barh(y, r['PP.H4'], color=colors[ds], height=0.72,
                    edgecolor='white', linewidth=0.4)
            ax.plot([r['PP.H4_p12_1e6'], r['PP.H4_p12_1e4']], [y, y], color='black',
...
plt.savefig('/mnt/results/P2-02_fig_位点内多基因coloc鉴别.png', dpi=300, bbox_inches='tight')
plt.savefig('/mnt/results/P2-02_fig_位点内多基因coloc鉴别.svg', bbox_inches='tight')
plt.show()
print('figure saved')